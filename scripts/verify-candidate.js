...
import { readJson, sha256 } from "./file-utils.js";
...
export function verifyCandidate(candidateDirectory) {
	...
	assert(
		sha256(path.join(ROOT, "postgresql-binaries.lock.json")) ===
			manifest.sourceLock.sha256,
		"candidate source lock differs from the checked-out source",
	);
	assert(
		sha256(path.join(ROOT, "SECURITY.md")) === manifest.securityPolicy.sha256,
...
function inspectTarball(tarball, expected) {
	const members = tar(tarball, ["-tzf", tarball]).split("\n").filter(Boolean);
