...
export async function createEphemeralCluster({ user = "postgres" } = {}) {
	...
	try {
		...
		run(
			pgCtl,
			["-D", dataDir, "-w", "-l", serverLog, "start"],
			// A persistent postgres.exe can inherit spawnSync's capture pipes and
			// prevent them from reaching EOF after pg_ctl exits. PostgreSQL already
			// redirects the server process to server.log.
			isWindows ? { stdio: "ignore", windowsHide: true } : undefined,
		);
	} catch (error) {
